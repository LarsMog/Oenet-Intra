...
/**
 * Methods supporting a list of Larsmog_events records.
 *
 * @since  1.6
 */
class IntraModelEvents extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	protected function getListQuery()
	{
		...
		$filter_Qdate_from = date('Y-m-d', $time);
		$filter_Qdate_to = date('Y-m-d', strtotime($filter_Qdate_from.' + 6 days'));
		...
		$query->from('`#__intra_events` AS a');
		...
		if (!JFactory::getUser()->authorise('core.edit', 'com_intra_events'))
		{
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('( a.place LIKE ' . $search . ' )');
			}
		}
