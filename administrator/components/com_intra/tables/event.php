 ...
 * @since  1.6
 */
class IntraTableevent extends JTable
{
	...
	{
		JObserverMapper::addObserverClassToClass('JTableObserverContenthistory', 'IntraTableevent', array('typeAlias' => 'com_intra.event'));
		parent::__construct('#__intra_events', 'id', $db);
	}
	...
	public function bind($array, $ignore = '')
	{
		...
		if (!JFactory::getUser()->authorise('core.admin', 'com_intra.event.' . $array['id']))
		{
			$actions         = JAccess::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_intra/access.xml',
				"/access/section[@name='event']/"
			);
			$default_actions = JAccess::getAssetRules('com_intra.event.' . $array['id'])->getData();
			$array_jaccess   = array();
	...
	public function check()
	{
		...
		if ($files['file'][0]['size'] > 0)
		{
			...
			$oldFiles = Larsmog_eventsHelpersLarsmog_events::getFiles($this->id, $this->_tbl, 'file');

			foreach ($oldFiles as $f)
			{
				$oldFile = JPATH_ROOT . '///' . $f;
			...
			foreach ($files['file'] as $singleFile )
			{
				...
				else
				{
					...
					$uploadPath = JPATH_ROOT . '///' . $filename;
	...
	protected function _getAssetName()
	{
		...
		return 'com_intra.event.' . (int) $this->$k;
	}
	...
	protected function _getAssetParentId(JTable $table = null, $id = null)
	{
		...
		$assetParent->loadByName('com_intra');
	...
	public function delete($pk = null)
	{
		...
		if ($result)
		{
			...
			{
				JFile::delete(JPATH_ROOT . '///' . $fileFile);
			}
		}
